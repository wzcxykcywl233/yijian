...
数据集整片图像外侧常带有较宽的白色衬底。脚本默认会先自动裁掉近白外边框，再用真实简片区域做 mask、inpainting 和 patch 裁剪，避免后续背景库学到白色画布。
...
- `--white_threshold`：白边检测阈值，默认 245。
- `--crop_margin`：裁剪真实图像区域时保留的边距，默认 2。

## 风险控制

- XML 损坏：只解析 `<bndbox>`，不依赖 `<name>`。
- 字符残留：通过 `mask_pad`、`dilate` 和 inpaint 半径控制。
- 白色衬底污染背景库：默认先裁掉近白外边框，并同步平移 XML 框坐标。
